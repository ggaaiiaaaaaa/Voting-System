...
<div class="flex min-h-screen">
    ...
    <aside class="w-64 bg-white shadow-lg fixed h-screen flex flex-col">
        <div class="p-6 border-b">
            ...
            <p class="text-xs text-gray-500 mt-1">Position Management</p>
        </div>
        ...
        <nav class="flex-1 overflow-y-auto mt-4">
            <ul class="space-y-1">
                ...
                <li><a href="view_position.php" class="flex items-center gap-3 px-6 py-2 bg-[#FEEAEA] text-[#D02C4D] font-medium">📌 Positions</a></li>
    ...
    <main class="flex-1 ml-64 p-8">
        ...
        <header class="flex justify-between items-center mb-6">
            <div>
                <h2 class="text-2xl font-semibold text-[#D02C4D]">Positions</h2>
            ...
            <table class="w-full text-sm text-left border border-gray-200">
                <thead class="bg-[#FEEAEA] text-[#D02C4D]">
                    <tr>
                        ...
                        <th class="px-4 py-3">Position Order</th>
                        <th class="px-4 py-3">Max Nominees</th>
                    ...
                    <?php if (!empty($positions)): ?>
                        <?php $i = 1; foreach ($positions as $pos): ?>
                            <tr class="hover:bg-gray-50">
                                ...
                                <td class="px-4 py-3"><?= htmlspecialchars($pos['position_name']) ?></td>
                                <td class="px-4 py-3"><?= htmlspecialchars($pos['position_order']) ?></td>
                                <td class="px-4 py-3"><?= htmlspecialchars($pos['max_nominees']) ?></td>
                                <td class="px-4 py-3">
                                    ...
                                    <?php else: ?>
                                        <span class="bg-red-100 text-red-700 px-2 py-1 rounded-full text-xs font-semibold"><?= htmlspecialchars($pos['status']) ?></span>
                                    <?php endif; ?>
                                </td>
                                <td class="px-4 py-3 flex justify-center gap-2">
                                    <a href="edit_position.php?id=<?= $pos['id'] ?>" class="bg-[#D02C4D] hover:bg-[#A0223B] text-white px-3 py-1 rounded text-xs font-medium">Edit</a>
                                    <a href="delete_position.php?id=<?= $pos['id'] ?>" onclick="return confirm('Are you sure you want to delete this position?')" class="bg-[#A0223B] hover:bg-[#D02C4D] text-white px-3 py-1 rounded text-xs font-medium">Delete</a>
                                </td>
                            </tr>
                        <?php endforeach; ?>
                    <?php else: ?>
                        <tr>
                            <td colspan="6" class="text-center py-6 text-gray-500">No positions found.</td>
                        </tr>
                    <?php endif; ?>
                </tbody>
            </table>
